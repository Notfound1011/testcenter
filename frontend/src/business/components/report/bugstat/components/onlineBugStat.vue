...
export default {
  name: "onlineBugStat",
  props: ['id', 'onlineBugJQL','groupData','group1','group2','group3','group4','group5','groupTotalData'],
  data() {
    return {
  ...
  methods: {
    onlineBugStat(data) {
      ...
      that.qaBugCreatedBarChart = that.$echarts.init(document.getElementById(that.id));

      let datas = groupArray(data, 'customfield_10406')
      let xAxisData = []
      let totalData = []
      //定义QA的四个组和组内成员
      ...
      that.qaBugCreatedBarChart.on('click', 'series.bar', obj => {
        let url = that.jira_address + "/issues/?jql=" + this.onlineBugJQL + " AND Tester"
